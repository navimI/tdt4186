...
struct priotable priotable;//priority table
...
void check_pri_Update(int ticks_proc)
{
    acquire(&priotable.lock);
    int i = 0;
...
int kill(int pid)
{
    ...
    for(p = proc; p < priotable.priCount[0]; p++)
    {
        acquire(&p->lock);
        if (p->pid == pid)
        {
